...
\ +022   (User Variable Pointer)
HERE TO vars^       R0 @ ,       \ HEX EAE0    ,


\ +024  (Return Stack Pointer)
...
\ 612Eh
\ +02E Execute "xt" i.e. CFA hold in HL
    HERE TO exec^
        LD      E'| (HL)|
        INCX    HL|
        LD      D'| (HL)|
...
\ This procedure does not go beyond a 'nul' ASCII that represents
...
CODE enclose    ( a c -- a  n1 n2 n3 )
        ...
        JRF    NZ'| HOLDPLACE ( no more character in string )
            LDN     D'|    0 N,
            INC     E'|
            PUSH    DE|
            DEC     E'|
        ...
        LDN     D'|    0 N,            ( separator & terminator )
        PUSH    DE|
        PUSH    DE|
        Next
        C;
...
\ Compute  HLDE := HL * DE, using the following algorithm: 
\    DE *      a subroutine is used to compute AHL := DE * A where
\    HL =      A keeps both L and result as soon as L's bits are spent.
\  ----        B is used as bit counter; C, D and E stay unchanged
\  0AHL [1]    compute partial AHL[1] := L * DE (and preserve H in C)
\  AHL  [2]    then save partial A[1] in C and partial HL[1] in stack
\  ----        partial L[1] already holds its final result
\  AHL  [3]    compute partial AHL[2] := H * DE
\  HLDE        compute partial AHL[3] := AHL[2] + 0AH[1]
\

\   Subroutine to compute AHL := DE * A
\   Register B is used as bit counter, CDE are unchanged
\   Register A both keeps L and, as L's bits are spent, buffers the result.
         
        HERE DUP      \ subroutine address used by the next two CALL AA,
        ASSEMBLER 
        LDX     HL|    0 NN,
        LDN     B'|    8 N,
        HERE \ BEGIN,
            ADDHL   HL|
            RLA
            JRF    NC'| HOLDPLACE   \ prepares holdplace for THEN,
                ADDHL   DE|
                ADCN    0   N,
            HERE DISP, \ THEN,
        DJNZ    HOLDPLACE SWAP DISP, \ djnz to BEGIN,
        RET
...
CODE um* ( u1 u2 -- ud )
        ...
        LD      C'|    H| 
        LD      A'|    L|
        CALL    AA,         \ address given by HERE DUP above
        PUSH    HL|
        LD      H'|    A|   \ swaps A and C (i.e. original H)
        LD      A'|    C|   \ using B as temp
        LD      C'|    H|   \ C saves partial A[1]
        CALL    AA,         \ address given by HERE DUP above
        POP     DE|         \ E holds its final result
        LD      B'|    C|   \ this is partial A[1]
        LD      C'|    D|   \ this is partial H[1]
        ADDHL   BC|
        ADCN    0 N,
        LD      D'|    L|
        LD      L'|    H|
...
\ 6495h
\ SP!
\ restore SP to the initial value, i.e. the content of
\ the offset 6 and 7 of user variabiles area.
CODE sp! ( -- )
...
\ 64B9h
\ RP!
\ restore RP to the initial value, i.e. the content of
\ the offset 8 and 9 of user variabiles area.
CODE rp! ( -- )
...
\ new
\ BANK!
\ 128K RAM bank switch: it maps n-th bank at C000-FFFF
...
\ For 128K BANK paging purpose, SP must be <= C000
: ?stack  ( -- )
    sp@ 
    s0 @ swap u< 1 ?error
    sp@
...
\ so new definitions can be inserted in tha vocabulary.
: vocabulary  ( -- cccc )
    <builds
        [ hex A081 ] literal ,
        current @ cfa ,
...
: cold  ( -- )
    ...
    [ vars^          ] Literal @  
    [ decimal    6   ] Literal +    
    [ decimal   16   ] Literal    \ this includes voc-link
    cmove
...
: mdrr1  ( n -- )
    ...
    chnl @ >r
    dup r [ decimal 13 ] Literal + c@ - \ CHREC of channel
    If
        1 - r [ decimal 13 ] Literal + c! 
        mdrget
    Else
        drop 
    Endif
...
\ SAVE "forth" CODE display
DECIMAL  0 +ORIGIN  DUP U.  FENCE @ SWAP - U. 
\ SAVE "forth" CODE display
\ ______________________________________________________________________ 

\ this cuts LFA so dictionary starts with "lit"
0 ' LIT 2 - ! final_rp_patch BYE BYE !
...
\ 63FF      RAMTOP  Logical RAM top
\
\ 6400              TIB @
\ 6450              Forth Return stack
\ 64B0              R0 @ - Forth User variables
\ 6500      ORIGIN  Forth Origin
\                   FENCE @
\                   LATEST @
\           HERE    DP @
\           PAD     HERE 68 + (44h)
